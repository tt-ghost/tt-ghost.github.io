      ...
      if (ptr) {
        ...
        for (var chn = 0; chn < num_channels; chn++) {
          ...
          for (var i = 0; i < num_frames; i++) {
            chan[i] =
              GROWABLE_HEAP_F32()[(ptr >> 2) + (num_channels * i + chn)];
          }
        }
      }
    };
...
function ___syscall_fstatat64(dirfd, path, buf, flags) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(4, 1, dirfd, path, buf, flags);
  try {
...
function registerPointerlockErrorEventCallback(
  target,
  userData,
  useCapture,
  callbackfunc,
...
function _fd_close(fd) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(34, 1, fd);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    FS.close(stream);
    return 0;
...
function _glDrawElements(mode, count, type, indices) {
  ...
  if (!GLctx.currentElementArrayBufferBinding) {
    ...
    GLctx.bufferSubData(
      34963,
      0,
      GROWABLE_HEAP_U8().subarray(indices, indices + size)
    );
    indices = 0;
  }
...
var __saudio_emsc_pull = (Module["__saudio_emsc_pull"] =
  createExportWrapper("_saudio_emsc_pull"));

var __sapp_emsc_notify_keyboard_hidden = (Module[
  "__sapp_emsc_notify_keyboard_hidden"
] = createExportWrapper("_sapp_emsc_notify_keyboard_hidden"));

var __sapp_emsc_onpaste = (Module["__sapp_emsc_onpaste"] =
  createExportWrapper("_sapp_emsc_onpaste"));

var __sapp_html5_get_ask_leave_site = (Module[
  "__sapp_html5_get_ask_leave_site"
] = createExportWrapper("_sapp_html5_get_ask_leave_site"));

var __sapp_emsc_begin_drop = (Module["__sapp_emsc_begin_drop"] =
  createExportWrapper("_sapp_emsc_begin_drop"));

var __sapp_emsc_drop = (Module["__sapp_emsc_drop"] =
  createExportWrapper("_sapp_emsc_drop"));

var __sapp_emsc_end_drop = (Module["__sapp_emsc_end_drop"] =
  createExportWrapper("_sapp_emsc_end_drop"));

var __sapp_emsc_invoke_fetch_cb = (Module["__sapp_emsc_invoke_fetch_cb"] =
  createExportWrapper("_sapp_emsc_invoke_fetch_cb"));
